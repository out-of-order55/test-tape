...
SCALA_EXT = scala
VLOG_EXT = sv v
FIRESIM_SOURCE_DIRS = $(addprefix $(base_dir)/sims/firesim/,sim/firesim-lib sim/midas/targetutils) $(addprefix $(base_dir)/generator/firechip/,chip bridgeinterfaces bridgestubs) $(dependencies_dir)/tools/firrtl2
...
#########################################################################################
CHISEL_STAMP ?= $(build_dir)/.chisel.stamp
$(FIRRTL_FILE) $(ANNO_FILE) $(CHISEL_LOG_FILE): $(CHISEL_STAMP)

$(CHISEL_STAMP): $(GENERATOR_CLASSPATH) $(EXTRA_GENERATOR_REQS) | $(build_dir)
	mkdir -p $(build_dir)
...
ANNO_STAMP ?= $(build_dir)/.anno.stamp
$(FINAL_ANNO_FILE) $(MFC_EXTRA_ANNO_FILE): $(ANNO_STAMP)

$(ANNO_STAMP): $(ANNO_FILE)
...
$(SFC_MFC_TARGETS): $(FIRTOOL_STAMP)

$(FIRTOOL_STAMP): $(FIRRTL_FILE) $(FINAL_ANNO_FILE) $(MFC_LOWERING_OPTIONS)
	...
	(set -o pipefail && $(FIRTOOL_BIN) \
			...
			--mlir-timing \
			--lowering-options=$$(cat $(MFC_LOWERING_OPTIONS)) \
			--repl-seq-mem \
...
UNIQUIFY_STAMP ?= $(build_dir)/.uniquify.stamp
...
SPLIT_MEMS_STAMP ?= $(build_dir)/.split-mems.stamp
...
TOP_MACRO_STAMP ?= $(build_dir)/.top-macrocompiler.stamp
...
MODEL_MACRO_STAMP ?= $(build_dir)/.model-macrocompiler.stamp
$(MODEL_SMEMS_FILE) $(MODEL_SMEMS_FIR): $(MODEL_MACRO_STAMP)
	@test -f $@ || { rm -f $(MODEL_MACRO_STAMP); $(MAKE) $(MODEL_MACRO_STAMP); }

$(MODEL_MACRO_STAMP): $(TAPEOUT_CLASSPATH) $(MODEL_SMEMS_CONF)
...
#######################################
dramsim_dir = $(dependencies_dir)/tools/DRAMSim2
