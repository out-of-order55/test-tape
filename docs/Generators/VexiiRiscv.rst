...
Configuration classes and Chipyard glue for VexiiRiscv live under ``soc-gen/generator/vexiiriscv/chipyard`` and are compiled directly from the VexiiRiscv repository sources. After running the standard setup (``./build-setup.sh``), build the example config with:

::

  cd soc-gen/sims/vcs && make CONFIG=VexiiRiscvConfig
