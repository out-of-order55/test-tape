...
def run_linux_poweroff():
    ...
    with prefix(f"cd {remote_fsim_dir}"):
        with prefix('source sourceme-manager.sh --skip-ssh-setup'):
            with prefix(f'cd {remote_cy_dir}/applications/firemarshal'):
            ...
            def run_w_timeout(workload_path, config_runtime, workload, timeout, num_passes):
                ...
                def run_firesim_cmd(typ, extra_args = ""):
                    ...
                    firesim_opts = f"-c {workload_path}/{config_runtime} -a {remote_cy_dir}/soc-generator/sims/firesim-staging/sample_config_hwdb.yaml -r {remote_cy_dir}/soc-generator/sims/firesim-staging/sample_config_build_recipes.yaml"
            ...
            run_w_timeout(f"{remote_cy_dir}/soc-generator/sims/firesim-staging/ci/{args.platform}", "config_runtime_rocket_singlecore.yaml", "linux-poweroff-singlenode-rocketsinglecore", "30m", 1)
            run_w_timeout(f"{remote_cy_dir}/soc-generator/sims/firesim-staging/ci/{args.platform}",   "config_runtime_rocket_quadcore.yaml",   "linux-poweroff-singlenode-rocketquadcore", "30m", 1)
            run_w_timeout(f"{remote_cy_dir}/soc-generator/sims/firesim-staging/ci/{args.platform}",   "config_runtime_boom_singlecore.yaml",   "linux-poweroff-singlenode-boomsinglecore", "30m", 1)
