...
class WithSystemModifications extends Config((site, here, up) => {
  ...
  case BootROMLocated(x) => up(BootROMLocated(x), site).map { p =>
    ...
    val make = s"make -C dep/fpga/src/main/resources/vcu118/sdboot PBUS_CLK=${freqMHz} bin"
    require (make.! == 0, "Failed to build bootrom")
    p.copy(hang = 0x10000, contentFileName = SystemFileName(s"./dep/fpga/src/main/resources/vcu118/sdboot/build/sdboot.bin"))
  }
