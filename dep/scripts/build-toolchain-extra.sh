...
SRCDIR="$(pwd)/dep/toolchains/${TOOLCHAIN}"
[ -d "${SRCDIR}" ] || die "unsupported toolchain: ${TOOLCHAIN}"
. ./dep/scripts/build-util.sh
...
(
    cd $RDIR
    git submodule update --init --checkout soc-gen/generator/constellation
    cd soc-gen/generator/constellation
...
CC= CXX= SRCDIR="$(pwd)/dep/toolchains" module_all libgloss --prefix="${RISCV}/riscv${XLEN}-unknown-elf" --host=riscv${XLEN}-unknown-elf

cd $RDIR
if [ $TOOLCHAIN == "riscv-tools" ]; then
    echo '==> Installing gemmini spike extensions'
    git submodule update --init soc-gen/generator/gemmini
    cd soc-gen/generator/gemmini
    git submodule update --init software/libgemmini
    make -C $RDIR/soc-gen/generator/gemmini/software/libgemmini install
fi

echo '==>  Installing DRAMSim2 Shared Library'
cd $RDIR
git submodule update --init dep/tools/DRAMSim2
cd dep/tools/DRAMSim2
make clean
make libdramsim.so
...
cd $RDIR
git submodule update --init soc-gen/generator/testchipip
...
cd $RDIR
git submodule update --init dep/toolchains/riscv-tools/riscv-spike-devices
cd dep/toolchains/riscv-tools/riscv-spike-devices
make install
