...
conda environment or \`source env.sh\` and skip this step with \`-s 1\`." >&2
    ...
    if [ "$USE_UNPINNED_DEPS" = true ]; then
        ...
        $CYDIR/dep/scripts/generate-conda-lockfiles.sh
        exit_if_last_command_failed
    fi
...
if run_step "3"; then
    ...
    $CYDIR/dep/scripts/build-toolchain-extra.sh $TOOLCHAIN_TYPE -p $PREFIX
    exit_if_last_command_failed
fi
...
if run_step "10"; then
	...
	$CYDIR/dep/scripts/build-circt-from-source.sh --prefix $PREFIX
    else
