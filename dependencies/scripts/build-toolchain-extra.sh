...
SRCDIR="$(pwd)/dependencies/toolchains/${TOOLCHAIN}"
[ -d "${SRCDIR}" ] || die "unsupported toolchain: ${TOOLCHAIN}"
. ./dependencies/scripts/build-util.sh
...
(
    cd $RDIR
    git submodule update --init --checkout soc-generator/generator/constellation
    cd soc-generator/generator/constellation
...
CC= CXX= SRCDIR="$(pwd)/dependencies/toolchains" module_all libgloss --prefix="${RISCV}/riscv${XLEN}-unknown-elf" --host=riscv${XLEN}-unknown-elf

cd $RDIR
if [ $TOOLCHAIN == "riscv-tools" ]; then
    echo '==> Installing gemmini spike extensions'
    git submodule update --init soc-generator/generator/gemmini
    cd soc-generator/generator/gemmini
    git submodule update --init software/libgemmini
    make -C $RDIR/soc-generator/generator/gemmini/software/libgemmini install
fi

echo '==>  Installing DRAMSim2 Shared Library'
cd $RDIR
git submodule update --init dependencies/tools/DRAMSim2
cd dependencies/tools/DRAMSim2
make clean
make libdramsim.so
...
cd $RDIR
git submodule update --init soc-generator/generator/testchipip
cd soc-generator/generator/testchipip/uart_tsi
make
cp uart_tsi $RISCV/bin

echo '==>  Installing spike-devices'
cd $RDIR
git submodule update --init dependencies/toolchains/riscv-tools/riscv-spike-devices
cd dependencies/toolchains/riscv-tools/riscv-spike-devices
make install
