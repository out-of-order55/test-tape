...

{
    ...
    if run_step 2; then
        ...
        "$CYDIR/dependencies/scripts/init-submodules-no-riscv-tools.sh" --full
    fi

    if run_step 3; then
        begin_step 3 "Building toolchain collateral"
        "$CYDIR/dependencies/scripts/build-toolchain-extra.sh" "$TOOLCHAIN_TYPE" -p "$RISCV"
    fi
    ...
    if run_step 10 && [ "$BUILD_CIRCT" = true ]; then
        ...
        "$CYDIR/dependencies/scripts/build-circt-from-source.sh" --prefix "$RISCV"
    fi
